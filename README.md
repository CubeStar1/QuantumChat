...
- The server doesn't handle encryption or decryption of messages. It simply relays messages between clients. The encryption and decryption are done client-side to ensure that even if someone intercepts the messages, they won't be able to read them without the correct symmetric key.
...
- This application consists of a Svelte frontend and a TypeScript backend.  
- The backend server makes requests to the key generation server to generate shared keys using quantum computations. 
